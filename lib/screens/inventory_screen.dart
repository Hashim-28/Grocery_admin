...
class _InventoryScreenState extends State<InventoryScreen> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() => context.read<DataProvider>().fetchProducts());
  }
          ...
          : filteredProducts.isEmpty
              ? Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.inventory_2_outlined, size: 64, color: AppTheme.textGrey.withOpacity(0.5)),
                      const SizedBox(height: 16),
                      const Text('No products found', style: TextStyle(color: AppTheme.textGrey, fontSize: 16)),
                    ],
                  ),
                )
              : ListView.builder(
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              image: product.coverImage != null
                  ? DecorationImage(
                      image: product.coverImage!.startsWith('http')
                          ? NetworkImage(product.coverImage!) as ImageProvider
                          : FileImage(File(product.coverImage!)),
                      fit: BoxFit.cover,
                    )
                  : null,
            ),
          ...
          Column(
            ...
            children: [
              Text(format.format(product.salePrice), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
